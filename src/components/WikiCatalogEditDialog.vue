<template>
  <el-dialog v-model="dialogVisible" :close-on-click-modal="true" :close-on-press-escape="true"
             :destroy-on-close="destroyOnClose" :draggable="true"
    ...
    <div id="accordion-flush" data-accordion="collapse"
      ...
      <h2 v-for="(catalog, index) in catalogs" id="accordion-flush-heading-1" :key="index">
        <!-- 一级目录 -->
        <button aria-controls="accordion-flush-body-1" aria-expanded="true"
                ...
                type="button">
          <svg aria-hidden="true" class="w-3 h-3 mr-2 rotate-180 shrink-0" data-accordion-icon
          ...
          <span v-else class="w-full">
                        <el-input v-model="catalog.title" clearable
                                  placeholder="请输入目录标题"
          ...
          <div class="hover:bg-gray-200 rounded py-2 px-2">
            <el-dropdown @command="handleCommand">
                            <span class="el-dropdown-link">
                                <svg class="icon w-5 h-5 outline-none" height="200" p-id="5968"
                                     t="1705472726484" version="1.1" viewBox="0 0 1024 1024" width="200"
        ...
        <ul v-if="catalog.children && catalog.children.length > 0">
          <VueDraggable ref="el" v-model="catalog.children" @end="onDragEnd">
            <li v-for="(childCatalog, index2) in catalog.children" :key="index2"
              ...
              <!-- 二级标题 -->
              <span class="w-full">
                                <span v-if="!childCatalog.editing" class="flex items-center"
                                      v-html="childCatalog.title"></span>
                                <span v-else>
                                    <el-input v-model="childCatalog.title" autofocus="true"
                                              clearable placeholder="请输入目录标题"
              ...
              <span class="hover:bg-gray-200 rounded py-2 px-2 ml-2 mr-2" @click="editTitle(childCatalog.id)">
                                <svg class="icon w-4 h-4" height="200" p-id="8306" t="1705474417719"
              ...
              <!-- 移出目录 -->
              <el-tooltip class="box-item" content="移出目录" effect="dark" placement="right">
                                <span class="hover:bg-gray-200 rounded py-2 px-2"
                                      @click="removeArticleFromCatalog(childCatalog.id)">
                                    <svg class="icon w-4 h-4" height="200" p-id="7174" t="1705473517473"
                                         version="1.1" viewBox="0 0 1024 1024" width="200"
                                         xmlns="http://www.w3.org/2000/svg">
                                        <path
  ...
  <!-- 添加目录 -->
  <FormDialog ref="addCatalogDialogRef" destroyOnClose title="添加目录" @submit="onAddCatalogSubmit">
    <el-form ref="addCatalogFormRef" :model="addCatalogForm" :rules="rules">
      <el-form-item label="标题" label-width="80px" prop="title" size="large">
        <el-input v-model="addCatalogForm.title" clearable placeholder="请输入目录标题"/>
      </el-form-item>
    </el-form>
  </FormDialog>

  <!-- 添加文章到目录 -->
  <FormDialog ref="addArticle2CatalogDialogRef" confirmText="添加" destroyOnClose title="添加文章" width="80%"
    ...
    <div>
      ...
      <el-card class="mb-5" shadow="never">
        ...
        <div class="flex items-center">
          ...
          <div class="ml-3 w-52 mr-5">
            <el-input v-model="searchArticleTitle" clearable placeholder="请输入（模糊查询）"/>
          </div>
          ...
          <div class="ml-3 w-30 mr-5">
            ...
            <el-date-picker v-model="pickDate" :shortcuts="shortcuts" end-placeholder="结束时间" range-separator="至"
                            size="default" start-placeholder="开始时间" type="daterange"
      ...
      <el-card shadow="never">
        ...
        <el-table v-loading="tableLoading" :data="tableData" border stripe @selection-change="handleSelectionChange">
          ...
          <el-table-column label="是否发布" prop="isPublish" width="100">
            <template #default="scope">
              <el-switch
...
import {Bottom, Check, Close, DocumentRemove, EditPen, Plus, RefreshRight, Search, Top} from '@element-plus/icons-vue'
...
const onAddCatalogSubmit = () => {
  ...
  addCatalogFormRef.value.validate((valid) => {
    ...
    // 获取最后一个一级目录对象
    let lastCatalog = catalogs.value && catalogs.value.length > 0 ? catalogs.value[catalogs.value.length - 1] : null;
    ...
    let newCatalog = {
      ...
      editing: false,
      level: 1,
      sort: lastCatalog ? lastCatalog.sort + 1 : 1, // 如果 lastCatalog 不存在，则从 1 开始排序
      // sort: lastCatalog.sort + 1, // 最后一个目录的 sort 值加一
      children: []
    }
    // 添加新目录
    console.log(newCatalog)
    if (Array.isArray(catalogs.value)) {
      catalogs.value.push(newCatalog)
    } else {
      catalogs.value = [newCatalog]
    }
    // catalogs.value.push(newCatalog)
