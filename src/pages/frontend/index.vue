  ...
  <main class="container max-w-screen-xl mx-auto px-4 md:px-6 py-8 min-h-[60vh]">
    ...
    <nav class="mb-8">
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <router-link aria-label="进入知识库"
                     class="group flex items-center gap-3 bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-xl p-4 shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-sky-500 focus:ring-offset-2 focus:ring-offset-white dark:focus:ring-offset-gray-800"
                     to="/wiki/list">
          <span
              class="flex h-10 w-10 items-center justify-center rounded-lg bg-sky-100 text-sky-600 dark:bg-sky-900/30 dark:text-sky-400">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path d="M12 6l-2-2m0 0L7 6m3-2v12m0-12l2 2m0 0l3-2m-5 2v12" stroke-linecap="round"
                    stroke-linejoin="round"
                    stroke-width="2"/>
            </svg>
          </span>
          <div class="flex-1">
            <div class="text-sm font-bold text-gray-800 dark:text-gray-100">知识库</div>
            <div class="text-xs text-gray-500 dark:text-gray-400">系统化阅读入口</div>
          </div>
          <svg class="w-5 h-5 text-gray-400 group-hover:text-sky-500 transition-colors" fill="none"
               stroke="currentColor" viewBox="0 0 24 24">
            <path d="M9 5l7 7-7 7" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"/>
          </svg>
        </router-link>

        <router-link aria-label="浏览分类"
                     class="group flex items-center gap-3 bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-xl p-4 shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2 focus:ring-offset-white dark:focus:ring-offset-gray-800"
                     to="/category/list">
          <span
          ...
          <div class="flex-1">
            <div class="text-sm font-bold text-gray-800 dark:text-gray-100">分类</div>
            <div class="text-xs text-gray-500 dark:text-gray-400">按主题快速查找</div>
          </div>
        ...
        <router-link aria-label="查看标签"
                     class="group flex items-center gap-3 bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-xl p-4 shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-rose-500 focus:ring-offset-2 focus:ring-offset-white dark:focus:ring-offset-gray-800"
                     to="/tag/list">
          <span
            ...
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path d="M7 7h10v10H7z" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"/>
            </svg>
          </span>
          <div class="flex-1">
            <div class="text-sm font-bold text-gray-800 dark:text-gray-100">标签</div>
            <div class="text-xs text-gray-500 dark:text-gray-400">按话题探索内容</div>
          </div>
          <svg class="w-5 h-5 text-gray-400 group-hover:text-rose-600 transition-colors" fill="none"
               stroke="currentColor" viewBox="0 0 24 24">
            <path d="M9 5l7 7-7 7" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"/>
          </svg>
        </router-link>
      ...
      <div class="col-span-1 lg:col-span-9 flex flex-col">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          ...
          <template v-if="loading">
            <div v-for="i in 6" :key="i"
                 class="bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-2xl overflow-hidden shadow-sm animate-pulse">
          ...
          <template v-else-if="articles && articles.length > 0">
            <article v-for="(article, index) in articles" :key="index" :style="{ animationDelay: `${index * 0.1}s` }"
              ...
              <router-link :aria-label="'查看文章：' + article.title"
                           :to="'/article/' + article.id"
                           class="relative w-full h-48 sm:h-56 overflow-hidden cursor-pointer focus:outline-none focus:ring-2 focus:ring-sky-500 focus:ring-offset-2 focus:ring-offset-white dark:focus:ring-offset-gray-800">
                <img :alt="article.title || '文章封面'" :src="article.cover"
              ...
              <div class="flex-1 p-5 flex flex-col">
                ...
                <!-- 优化：字号 text-xl -->
                <h2
                    class="text-xl font-bold text-gray-800 dark:text-white mb-2 line-clamp-2 hover:text-sky-600 dark:hover:text-sky-400 transition-colors cursor-pointer leading-snug"
                ...
                <div
                  ...
                  <button aria-label="阅读全文"
                          class="group-hover:text-sky-500 transition-colors flex items-center gap-0.5 cursor-pointer focus:outline-none focus:ring-2 focus:ring-sky-500 focus:ring-offset-2 focus:ring-offset-white dark:focus:ring-offset-gray-800"
                          type="button" @click="goArticleDetailPage(article.id)">
                    阅读全文
        ...
        <div v-if="pages > 1" class="flex justify-center mt-auto">
          <nav class="inline-flex rounded-md shadow-sm isolate">
            ...
            <!-- 页码 -->
            <button v-for="(pageNo, index) in pages" :key="index" :class="[
              pageNo === current
