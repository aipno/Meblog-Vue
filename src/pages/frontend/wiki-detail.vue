  ...
  <div class="main min-h-screen flex flex-col">
    ...
    <main class="grow container max-w-screen-3xl mx-auto px-4 sm:px-6 md:px-8 py-4">
      ...
      <div
        ...
        class="hidden lg:block fixed z-20 inset-0 top-[5.5rem]
                 right-auto w-[19rem] pb-10 pr-6 overflow-y-auto">

        <div class="flex">
          <!-- 知识库目录 -->
          <div :class="[isExpand ? 'block' : 'hidden 2xl:block']" class="grow">
            <div id="accordion-flush" class="last:pb-[170px]" data-accordion="collapse"
              ...
              <div v-for="(catalog, index) in catalogs" :key="index">
                <h2 :id="'accordion-flush-heading-' + catalog.id">
                  <button :aria-controls="'accordion-flush-body-' + catalog.id"
                    :aria-expanded="[!!(Array.isArray(catalog.children) && catalog.children.some(item => item.articleId === route.query.articleId))]"
                    :data-accordion-target="'#accordion-flush-body-' + catalog.id" class="hover:bg-gray-100 flex items-center justify-between w-full py-3 px-3 rounded-lg
                    ...
                    <!-- 箭头 -->
                    <svg aria-hidden="true" class="w-3 h-3 rotate-90 transition-all shrink-0" data-accordion-icon
                ...
                <!-- 二级目录 -->
                <ul :id="'accordion-flush-body-' + catalog.id"
                  :aria-labelledby="'accordion-flush-heading-' + catalog.id" class="hidden">
                  <!-- 二级目录标题 -->
                  <li v-for="(childCatalog, index2) in catalog.children" :key="index2"
                    ...
                    @click="goWikiArticleDetailPage(childCatalog.articleId)" v-html="childCatalog.title"></li>
                </ul>
              </div>
            </div>
          </div>
          ...
          <div class="hidden md:inline-block 2xl:hidden transition-all" @click="shrinkAndExpand">
            <div id="left-toc-sidebar" class="left-toc-sidebar top-[5.5rem]">
              <span id="left-toc-sidebar-arrow" :class="[isExpand ? '-rotate-90' : 'rotate-90']"
      ...
      <!-- 中间栏 -->
      <div :class="[isExpand ? 'lg:pl-[20rem]' : 'lg:pl-0 2xl:pl-[22.5rem]']" class="transition-all duration-300">
        <div class="max-w-3xl mx-auto xl:max-w-none xl:ml-0 xl:mr-[10rem] xl:pr-16">
          <!-- 文章 -->
          <article>
            <!-- 文章标题、Meta 信息 -->
            <div class="mt-5">
              ...
              <div class="flex gap-3 md:gap-6 text-gray-400 items-center text-sm pb-3
                ...
                <!-- 字数 -->
                <div class="flex items-center" data-tooltip-placement="bottom"
                  data-tooltip-target="word-tooltip-bottom">
                  <svg class="w-4 h-4 mr-1 icon" height="48" p-id="28617" t="1701512226243" version="1.1"
                    viewBox="0 0 1024 1024" width="48" xmlns="http://www.w3.org/2000/svg">
                    <path
                      ...
                      fill="#8a8a8a" p-id="28618"></path>
                  </svg>
                  {{ article.totalWords }}
                </div>
                <div id="word-tooltip-bottom"
                ...
                <div class="hidden md:block">
                  <div class="flex items-center" data-tooltip-placement="bottom"
                    ...
                    <svg class="w-4 h-4 mr-1.5 icon" height="48" p-id="37812" t="1701512553358" version="1.1"
                      viewBox="0 0 1024 1024" width="48" xmlns="http://www.w3.org/2000/svg">
                      <path
                  ...
                  <div id="read-time-tooltip-bottom"
                    class="absolute z-10 invisible inline-block px-3 py-2 text-xs font-medium text-white bg-gray-900 rounded shadow-sm opacity-0 tooltip dark:bg-gray-700"
                    role="tooltip">
                    阅读时长
                    <div class="tooltip-arrow" data-popper-arrow></div>
                ...
                <div class="flex items-center" data-tooltip-placement="bottom"
                  ...
                  <svg class="w-[18px] h-[18px] mr-1 icon" height="48" p-id="41600" t="1701513012543" version="1.1"
                    viewBox="0 0 1024 1024" width="48" xmlns="http://www.w3.org/2000/svg">
                    <path
                ...
                <div id="publish-time-tooltip-bottom"
                  class="absolute z-10 invisible inline-block px-3 py-2 text-xs font-medium text-white bg-gray-900 rounded shadow-sm opacity-0 tooltip dark:bg-gray-700"
                  role="tooltip">
                  发布时间
                  <div class="tooltip-arrow" data-popper-arrow></div>
                ...
                <div class="flex items-center" data-tooltip-placement="bottom"
                  ...
                  <svg class="w-4 h-4 mr-1.5 icon" height="48" p-id="50560" t="1701513357854" version="1.1"
                    viewBox="0 0 1024 1024" width="48" xmlns="http://www.w3.org/2000/svg">
                    <path
                ...
                <div class="flex items-center" data-tooltip-placement="bottom"
                  ...
                  <svg class="w-[18px] h-[18px] mr-1 icon" height="48" p-id="56112" t="1701513523793" version="1.1"
                    viewBox="0 0 1024 1024" width="48" xmlns="http://www.w3.org/2000/svg">
                    <path
                      ...
                      fill="#8a8a8a" p-id="56113"></path>
                  </svg>
                  {{ article.readNum }}
                </div>
                <div id="read-num-tooltip-bottom"
                  class="absolute z-10 invisible inline-block px-3 py-2 text-xs font-medium text-white bg-gray-900 rounded shadow-sm opacity-0 tooltip dark:bg-gray-700"
                  role="tooltip">
                  阅读量
                  <div class="tooltip-arrow" data-popper-arrow></div>
            ...
            <div :class="{ 'dark': isDark }">
              <div v-viewer class="article-content" v-html="article.content">
              </div>
            </div>

            <!-- 最后编辑时间 -->
            <div class="flex items-center text-gray-500 text-sm">
              <svg class="icon inline-block w-4 h-4 mr-1 mt-5 mb-5" height="200" p-id="7701" t="1705985882454"
                ...
                <path
                  ...
                  fill="#707070" p-id="7702"></path>
              </svg>
              最后编辑于 {{ article.updateTime }}
            </div>

            <!-- 上下篇 -->
            <nav v-if="preNext" class="flex flex-row mt-7">
              ...
              <div class="basis-1/2">
                ...
                <a v-if="preNext.preArticle"
                  ...
                  <div>
                    <svg aria-hidden="true" class="inline w-3.5 h-3.5 mr-2 mb-1" fill="none" viewBox="0 0 14 10"
              ...
              <div class="basis-1/2">
                ...
                <a v-if="preNext.nextArticle"
                  ...
                  <div>
                    下一篇
                    <svg aria-hidden="true" class="inline w-3.5 h-3.5 ml-2 mb-1" fill="none" viewBox="0 0 14 10"
                      xmlns="http://www.w3.org/2000/svg">
