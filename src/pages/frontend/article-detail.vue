<template>
  ...
  <!-- 文章标题、标签、Meta 信息 -->
  <div v-if="success" class="bg-white dark:bg-gray-900">
    <div class="max-w-screen-xl flex flex-col flex-wrap mx-auto px-4 md:px-6 pb-14 pt-10">
      ...
      <div v-if="article.tags && article.tags.length > 0" class="mb-5">
        <span v-for="(tag, index) in article.tags" :key="index" class="inline-block mb-1 cursor-pointer bg-green-100 text-green-800 text-sm font-medium me-2
      ...
      <div class="flex gap-3 md:gap-6 text-gray-400 items-center text-sm">
        <!-- 字数 -->
        <div class="flex items-center" data-tooltip-placement="bottom" data-tooltip-target="word-tooltip-bottom">
        ...
        <div class="hidden md:block">
          <div class="flex items-center" data-tooltip-placement="bottom" data-tooltip-target="read-time-tooltip-bottom">
        ...
        <!-- 分类 -->
        <div class="flex items-center" data-tooltip-placement="bottom" data-tooltip-target="category-tooltip-bottom">
          ...
          <a class="cursor-pointer mr-1 hover:underline"
            @click="goCategoryArticleListPage(article.categoryId, article.categoryName)">{{ article.categoryName }}</a>
        </div>
        <!-- 分类 Tooltip -->
        ...
        <!-- 阅读量 -->
        <div class="flex items-center" data-tooltip-placement="bottom" data-tooltip-target="read-num-tooltip-bottom">
    ...
    <div class="grid grid-cols-4 gap-7">
      ...
      <div class="col-span-4 md:col-span-3 mb-3">
        ...
        <div class="w-full p-5 mb-3 bg-white border border-gray-200 rounded-lg dark:bg-gray-800 dark:border-gray-700">
          ...
          <NoPermission v-if="articlePermission"></NoPermission>

          <!-- 上下篇 -->
<!--          <nav class="flex flex-row mt-7">-->
<!--            &lt;!&ndash; basis-1/2 用于占用 flex 布局的一半空间 &ndash;&gt;-->
<!--            <div class="basis-1/2">-->
<!--              &lt;!&ndash; h-full 指定高度占满 &ndash;&gt;-->
...
<!--            <div class="basis-1/2">-->
<!--              &lt;!&ndash; text-right 指定文字居右显示 &ndash;&gt;-->
...
function refreshArticleDetail(articleId) {
  getArticleDetail(route.params.articleId).then((res) => {
    // 如果是无权限错误，直接返回，不再处理 DOM
    if (!res.success && res.errorCode === '20002') {
